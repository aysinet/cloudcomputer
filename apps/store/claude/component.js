(function(Vue) {
  ...
  return {
    setup() {
      ...
      const iframeSrc = ref(proxyUrl('https://claude.ai/login'));

      function navigateHome() {
        iframeSrc.value = proxyUrl('https://claude.ai/login');
      }

      function goBack() {
        try {
          if (claudeFrame.value) claudeFrame.value.contentWindow.history.back();
        } catch(e) {}
      }

      function goForward() {
        try {
          if (claudeFrame.value) claudeFrame.value.contentWindow.history.forward();
        } catch(e) {}
      }
