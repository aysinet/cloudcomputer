(function(Vue) {
  ...
  const DOCKER_IMAGE = 'jellyfin/jellyfin:latest';
  const APP_ID = 'jellyfin';
  const CONTAINER_PORT = 8096;
  ...
  return {
    setup() {
      ...
      function getVolumes() {
        return [
          '${DATA_VOLUME}:/media:ro',
          '${APP_VOLUME}/config:/config',
          '${APP_VOLUME}/cache:/cache'
        ];
      }

      function getEnv() {
        return [
          'JELLYFIN_PublishedServerUrl=/proxy/jellyfin'
        ];
      }
      ...
      async function pullImage() {
        ...
        try {
          ...
          pullProgress.value = 'Image indirildi! Jellyfin başlatılıyor...';
          await checkAndStart();
        } catch (e) {
