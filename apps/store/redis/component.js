  ...
  return {
    setup() {
      ...
      async function restart() {
        ...
        try {
          ...
          const body = {
            ...
            appId: APP_ID,
            containerPort: manifest.docker.containerPort || 6379,
            volumes: manifest.docker.volumes || [],
            env,
            restart: 'always'
          };
          if (Array.isArray(cfg.cmd)) body.cmd = cfg.cmd;
      ...
      const connInfo = computed(() => {
        ...
        const hasPass = serviceInfo.value.installConfig && serviceInfo.value.installConfig.REDIS_PASSWORD;
        return {
          host: 'localhost',
